...
try:
    import reaper_mcp_activity as _reaper_activity
except ImportError:
    _reaper_activity = None
...
class LoggedFastMCP(FastMCP):
    """FastMCP with optional activity logging for the localhost dashboard."""

    async def call_tool(self, name: str, arguments: dict[str, Any]) -> Any:
        act = _reaper_activity
        if act is None:
            return await super().call_tool(name, arguments)
        rid = act.log_tool_start(name, arguments)
        t0 = time.time()
        try:
            result = await super().call_tool(name, arguments)
            act.log_tool_end(rid, name, True, time.time() - t0, result=result)
            return result
        except Exception as e:
            act.log_tool_end(rid, name, False, time.time() - t0, error=str(e))
            raise


# Create MCP server
mcp = LoggedFastMCP("twelvetake-reaper-mcp")
...
async def reaper_call(func: str, *args) -> dict:
    ...
    if COMM_MODE == "file":
        result = await reaper_call_file(func, args_list)
    elif COMM_MODE == "http":
    ...
    if _reaper_activity and _reaper_activity.bridge_log_enabled():
        elapsed = time.time() - t0
        summ = "ok"
        if isinstance(result, dict) and result.get("ok") is False:
            summ = str(result.get("error", "fail"))[:500]
        _reaper_activity.log_bridge_call(func, args_list, elapsed, summ)
...
def _render_reaper_install_guide(section: str) -> str:
    path = Path(__file__).resolve().parent / "install_help.py"
    spec = importlib.util.spec_from_file_location("reaper_mcp_install_help", path)
    if spec is None or spec.loader is None:
        return "install_help.py missing next to reaper_mcp_server.py"
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.render_reaper_install_guide(section)


@mcp.tool()
async def get_reaper_mcp_install_guide(section: str = "full") -> str:
    """
    Return step-by-step installation: REAPER Lua bridge, Python deps, Cursor MCP JSON,
    file vs HTTP mode, dashboard env vars, verification. Does not call REAPER.

    Args:
        section: full (default), reaper_bridge, python, cursor, http, verify, dashboard
    """
    return _render_reaper_install_guide(section)
...
async def add_midi_note(
    ...
    return await reaper_call(
        "BridgeMIDI_InsertNoteTrackItem",
        track_index,
        item_index,
        False,
        False,
...
@mcp.tool()
async def ensure_track_envelope(track_index: int, envelope_name: str) -> dict:
    """
    Enable a built-in track envelope (Volume, Pan) if it does not exist yet.

    REAPER only exposes Volume/Pan envelopes after they are active; this runs
    the same toggle as the track envelope menu so GetTrackEnvelopeByName works.
    """
    r = await reaper_call("EnsureTrackEnvelope", track_index, envelope_name)
    if r.get("ok"):
        return r
    err = str(r.get("error", ""))
    if "Unknown function" not in err:
        return r
    if envelope_name not in ("Volume", "Pan"):
        return {"ok": False, "error": "ensure fallback supports Volume and Pan only"}
    await reaper_call("SetOnlyTrackSelected", track_index)
    ac = 40052 if envelope_name == "Volume" else 40053
    await reaper_call("Main_OnCommand", ac, 0)
    return {"ok": True, "ret": True, "fallback": True}
...
async def add_envelope_point(
    ...
    """
    return await reaper_call(
        "InsertTrackEnvelopePointByName",
        track_index,
        envelope_name,
        time,
        value,
...
@mcp.tool()
async def add_envelope_points_batch(
    track_index: int,
    envelope_name: str,
    points: list,
) -> dict:
    """
    Insert many envelope points in one bridge round-trip (faster than repeated add_envelope_point).

    Args:
        track_index: Track index (0-based) or -1 for master.
        envelope_name: Envelope name (e.g. Volume, Pan).
        points: List of dicts with keys: time (seconds), value (0-1 typical), shape (optional, default 0),
                tension (optional), selected, noSort.

    Returns:
        ok, inserted count, failed count.
    """
    r = await reaper_call(
        "InsertTrackEnvelopePointsBatch",
        track_index,
        envelope_name,
        points,
    )
    if r.get("ok"):
        return r
    err = str(r.get("error", ""))
    if "InsertTrackEnvelopePointsBatch" not in err or "Unknown function" not in err:
        return r
    inserted = 0
    failed = 0
    for pt in points:
        ...
        v = pt.get("value", pt.get("v"))
        if not isinstance(t, (int, float)) or not isinstance(v, (int, float)):
            failed += 1
            continue
        shape = int(pt.get("shape", pt.get("s", 0)))
        rr = await reaper_call(
            "InsertTrackEnvelopePointByName",
            track_index,
            envelope_name,
            float(t),
            float(v),
            shape,
            float(pt.get("tension", 0.0)),
            bool(pt.get("selected", False)),
            bool(pt.get("noSort", False)),
        )
        if rr.get("ok"):
            inserted += 1
        else:
            failed += 1
    return {"ok": True, "inserted": inserted, "failed": failed, "fallback": True}


@mcp.tool()
async def add_fx_envelope_points_batch(
    track_index: int,
    fx_index: int,
    param_index: int,
    points: list,
    ...
    """
    Insert automation points for a VST/AU/CLAP FX parameter (uses REAPER GetFXEnvelope).

    Args:
        track_index: Track index (0-based) or -1 for master.
        fx_index: FX slot index in the chain (0 = first).
        param_index: Parameter index (0-based).
        points: List of dicts with time (seconds), value (0-1), optional shape, tension.

    Returns:
        ok, inserted, failed counts.
    """
    r = await reaper_call(
        "InsertFXEnvelopePointsBatch",
        track_index,
        fx_index,
        param_index,
        points,
...
@mcp.tool()
async def list_fx_parameters(
    track_index: int,
    fx_index: int,
    max_params: int = 512,
) -> dict:
    """
    List FX parameter index and display name for one plugin slot.

    Same ordering as REAPER's envelope panel (e.g. Sequencer:Range, Compressor:Drive on Reaktor).
    Use the index with add_fx_envelope_points_batch, or use find_fx_param_indices / add_fx_envelope_points_by_param_name.
    """
    c = await track_fx_get_num_params(track_index, fx_index)
    if not c.get("ok"):
        return {"ok": False, "error": c.get("error", "TrackFX_GetNumParams failed"), "params": []}
    n = int(c.get("ret", 0))
    params: list[dict] = []
    for pi in range(min(n, max_params)):
        nm = await track_fx_get_param_name(track_index, fx_index, pi)
        params.append({"index": pi, "name": nm.get("ret") or ""})
    return {"ok": True, "fx_index": fx_index, "param_count": n, "params": params}


@mcp.tool()
async def find_fx_slot_by_name(track_index: int, name_contains: str) -> dict:
    """
    Find FX chain slots whose plugin name contains the substring (case-insensitive), e.g. "Reaktor".
    """
    needle = (name_contains or "").lower()
    cnt = await track_fx_get_count(track_index)
    if not cnt.get("ok"):
        return {"ok": False, "error": cnt.get("error"), "matches": []}
    nfx = int(cnt.get("ret", 0))
    matches: list[dict] = []
    for i in range(nfx):
        r = await track_fx_get_name(track_index, i)
        disp = r.get("ret") or ""
        if needle in disp.lower():
            matches.append({"fx_index": i, "name": disp})
    return {"ok": True, "matches": matches}


@mcp.tool()
async def find_fx_param_indices(
    track_index: int,
    fx_index: int,
    name_contains: str,
    max_params: int = 1024,
    ...
    """
    Find parameter indices whose display name contains name_contains (case-insensitive).

    Targets the same names as in the TCP envelope list (e.g. "Sequencer:Paste", "Compressor:Att").
    """
    needle = (name_contains or "").lower()
    c = await track_fx_get_num_params(track_index, fx_index)
    if not c.get("ok"):
        return {"ok": False, "error": c.get("error"), "matches": []}
    n = int(c.get("ret", 0))
    matches: list[dict] = []
    for pi in range(min(n, max_params)):
        nm = await track_fx_get_param_name(track_index, fx_index, pi)
        ...
        if needle in name.lower():
            matches.append({"param_index": pi, "name": name})
    return {"ok": True, "fx_index": fx_index, "matches": matches}


@mcp.tool()
async def add_fx_envelope_points_by_param_name(
    track_index: int,
    fx_index: int,
    name_contains: str,
    points: list,
    ...
    """
    Write FX automation to the first parameter (or all) whose name contains name_contains.

    Uses the same GetFXEnvelope targets as envelopes shown for that VST in the track panel.
    """
    needle = (name_contains or "").lower()
    c = await track_fx_get_num_params(track_index, fx_index)
    if not c.get("ok"):
        return {"ok": False, "error": c.get("error"), "applied": []}
    n = int(c.get("ret", 0))
    ...
    for pi in range(min(n, max_params)):
        ...
        name = nm.get("ret") or ""
        if needle in name.lower():
            targets.append((pi, name))
            if not match_all:
                break
    if not targets:
        return {
            "ok": False,
            "error": "No parameter name contains %r" % name_contains,
            "applied": [],
        }
    applied: list[dict] = []
    for pi, name in targets:
        r = await add_fx_envelope_points_batch(track_index, fx_index, pi, points)
        applied.append(
            {
                "param_index": pi,
                "name": name,
    ...
    if any(int(a.get("inserted") or 0) > 0 for a in applied):
        await ensure_automation_audible()
    return {"ok": True, "applied": applied, "match_count": len(applied)}
...
@mcp.tool()
async def ensure_automation_audible() -> dict:
    """
    If global automation is in Bypass mode (override 5), reset to no override (-1)
    and refresh the arrange so VST envelopes actually affect playback.
    """
    r = await reaper_call("EnsureAutomationAudible")
    if r.get("ok"):
        return r
    if "Unknown function" not in str(r.get("error", "")):
        return r
    await reaper_call("SetGlobalAutomationOverride", -1)
    await reaper_call("UpdateArrange")
    await reaper_call("UpdateTimeline")
...
def main():
    ...
    if _reaper_activity is not None:
        _reaper_activity.maybe_autostart_dashboard()
    mcp.run()
