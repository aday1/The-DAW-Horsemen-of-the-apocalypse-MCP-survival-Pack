...
Call **`get_reaper_mcp_install_guide`** from your MCP client (`section`: `full`, `reaper_bridge`, `python`, `cursor`, `http`, `verify`, or `dashboard`). It returns Markdown and does not need REAPER to respond. Use it to paste Cursor config and path reminders, then run **`test_connection.py`** or **`get_project_summary`**.
