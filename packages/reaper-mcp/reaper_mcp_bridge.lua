            ...
            local ok, err = pcall(function()
                ...
                if request_data then
                    ...
                    elseif fname == "AddProjectMarker" then
                        if #args >= 5 then
                            local index = reaper.AddProjectMarker(args[1], args[2], args[3], args[4], args[5])
                            response.ok = true
                            response.ret = index
                        else
                    ...
                    elseif fname == "EnsureTrackEnvelope" then
                        if #args >= 2 then
                            local track_idx = args[1]
                            local env_name = args[2]
                            local track = nil
                            if track_idx == -1 then
                                track = reaper.GetMasterTrack(0)
                            else
                            ...
                            if not track then
                                ...
                                if env then
                                    ...
                                    if env then
                                        ...
                                        response.ok = false
                                        response.error = "Could not enable envelope: " .. tostring(env_name)
                                    end
                                end
                            end
                        else
                            response.error = "EnsureTrackEnvelope requires track_index, envelope_name"
                            response.ok = false
                        end
                    ...
                    elseif fname == "ClearEnvelope" then
                        if #args >= 2 then
                            ...
                            response.error = "ClearEnvelope requires track_index, envelope_name"
                            response.ok = false
                        end
                    
                    elseif fname == "InsertTrackEnvelopePointByName" then
                        if #args >= 8 then
                            local track_idx = args[1]
                            local env_name = args[2]
                            local etime = args[3]
                            ...
                            if not track then
                                ...
                                local env = reaper.GetTrackEnvelopeByName(track, env_name)
                                if not env then
                                    response.error = "Envelope not found: " .. tostring(env_name)
                                    response.ok = false
                                else
                                    local result = reaper.InsertEnvelopePoint(env, etime, evalue, shape, tension, selected, noSort)
                                    response.ok = result
                                    response.ret = result
                                end
                            end
                        else
                            response.error = "InsertTrackEnvelopePointByName requires 8 arguments"
                            response.ok = false
                        end
                    
                    elseif fname == "InsertTrackEnvelopePointsBatch" then
                        if #args >= 3 then
                            local track_idx = args[1]
                            local env_name = args[2]
                            local points = args[3]
                            ...
                            elseif type(points) ~= "table" then
                                response.error = "points must be a list of objects"
                                response.ok = false
                            else
                                local env = reaper.GetTrackEnvelopeByName(track, env_name)
                                if not env then
                                    ...
                                    for _, pt in ipairs(points) do
                                        if type(pt) == "table" then
                                            local etime = pt.time or pt.t
                                            local evalue = pt.value or pt.v
                                            local shape = pt.shape or pt.s or 0
                                            local tension = pt.tension or 0.0
                                            local selected = pt.selected or false
                                            local noSort = pt.noSort or false
                                            if type(etime) == "number" and type(evalue) == "number" then
                                                local ok = reaper.InsertEnvelopePoint(env, etime, evalue, shape, tension, selected, noSort)
                                                if ok then
                                                    inserted = inserted + 1
                                                else
                                                    failed = failed + 1
                            ...
                            response.error = "InsertTrackEnvelopePointsBatch requires track_index, env_name, points"
                            response.ok = false
                        end
                    
                    elseif fname == "EnsureAutomationAudible" then
                        local prev = reaper.GetGlobalAutomationOverride()
                        if prev == 5 then
                            reaper.SetGlobalAutomationOverride(-1)
                            response.cleared_automation_bypass = true
                        end
                        reaper.UpdateArrange()
                        ...
                        response.ok = true
                        response.previous_global_automation_override = prev
                    
                    elseif fname == "InsertFXEnvelopePoint" then
                        if #args >= 7 then
                            local track_idx = args[1]
                            local fx_idx = math.floor(tonumber(args[2]) or 0)
                            local param_idx = math.floor(tonumber(args[3]) or 0)
                            local etime = args[4]
                            local evalue = args[5]
                            local shape = args[6]
                            local tension = args[7]
                            local track = nil
                            local tin = tonumber(track_idx)
                            if tin and tin < 0 then
                                track = reaper.GetMasterTrack(0)
                            elseif tin then
                                track = reaper.GetTrack(0, math.floor(tin + 0.5))
                            end
                            if not track then
                                response.error = "Track not found"
                                response.ok = false
                            elseif not reaper.GetFXEnvelope then
                                response.error = "GetFXEnvelope not in this REAPER build"
                                response.ok = false
                            else
                                local env = reaper.GetFXEnvelope(track, fx_idx, param_idx, true)
                                if not env then
                                    response.error = "GetFXEnvelope failed (fx " .. tostring(fx_idx) .. " param " .. tostring(param_idx) .. ")"
                                    response.ok = false
                                else
                                    local ok = reaper.InsertEnvelopePoint(env, etime, evalue, shape, tension, false, false)
                                    if ok and reaper.Envelope_SortPoints then
                                        reaper.Envelope_SortPoints(env)
                                    end
                                    if ok then
                                        local ov = reaper.GetGlobalAutomationOverride()
                                        if ov == 5 then
                                            reaper.SetGlobalAutomationOverride(-1)
                                        end
                                        reaper.UpdateArrange()
                                    end
                                    response.ok = ok and true or false
                                    response.ret = ok
                                end
                            end
                        else
                            response.error = "InsertFXEnvelopePoint requires track, fx, param, time, value, shape, tension"
                            response.ok = false
                        end
                    
                    elseif fname == "InsertFXEnvelopePointsBatch" then
                        if #args >= 4 then
                            local track_idx = args[1]
                            local fx_idx = math.floor(tonumber(args[2]) or 0)
                            local param_idx = math.floor(tonumber(args[3]) or 0)
                            ...
                            elseif not reaper.GetFXEnvelope then
                                ...
                                response.ok = false
                            elseif type(points) ~= "table" then
                                response.error = "points must be a list"
                                response.ok = false
                            else
                                local env = reaper.GetFXEnvelope(track, fx_idx, param_idx, true)
                                if not env then
                                    ...
                                    for _, pt in ipairs(points) do
                                        if type(pt) == "table" then
                                            local etime = pt.time or pt.t
                                            local evalue = pt.value or pt.v
                                            local shape = math.floor(tonumber(pt.shape or pt.s or 0) or 0)
                                            local tension = tonumber(pt.tension) or 0.0
                                            if type(etime) == "number" and type(evalue) == "number" then
                                                local ok = reaper.InsertEnvelopePoint(env, etime, evalue, shape, tension, false, false)
                                                if ok then
                                                    inserted = inserted + 1
                            ...
                            response.error = "InsertFXEnvelopePointsBatch requires track, fx, param, points"
                            response.ok = false
                        end
                    ...
                    elseif fname == "BridgeMIDI_InsertNoteTrackItem" then
                        -- track_index, item_index, selected, muted, start_ppq_rel, end_ppq_rel, channel, pitch, velocity, noSort
                        if #args >= 10 then
                            local track_idx = args[1]
                            local item_idx = args[2]
                            local selected = args[3]
                            local muted = args[4]
                            local rel_start_ppq = args[5]
                            local rel_end_ppq = args[6]
                            local channel = args[7]
                            local pitch = args[8]
                            local velocity = args[9]
                            local noSort = args[10]
                            local track = nil
                            if type(track_idx) == "number" and track_idx == -1 then
                                track = reaper.GetMasterTrack(0)
                            elseif type(track_idx) == "number" then
                                track = reaper.GetTrack(0, track_idx)
                            end
                            if not track then
                                response.error = "Track not found"
                                ...
                                if not item then
                                    ...
                                    local take = reaper.GetActiveTake(item)
                                    if not take or not reaper.TakeIsMIDI(take) then
                                        response.error = "Not a MIDI take"
                                        response.ok = false
                                    else
                                        local item_pos = reaper.GetMediaItemInfo_Value(item, "D_POSITION")
                                        local base_ppq = reaper.MIDI_GetPPQPosFromProjTime(take, item_pos)
                                        local ppq0 = base_ppq + rel_start_ppq
                                        local ppq1 = base_ppq + rel_end_ppq
                                        local result = reaper.MIDI_InsertNote(take, selected, muted, ppq0, ppq1, channel, pitch, velocity, noSort)
                                        response.ok = result
                                        if not result then
                                            response.error = "BridgeMIDI_InsertNoteTrackItem failed"
                                        end
                                    end
                                end
                            end
                        else
                            response.error = "BridgeMIDI_InsertNoteTrackItem requires 10 arguments"
                            response.ok = false
                        end
